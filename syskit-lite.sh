...
show_fastfetch() {
    if command -v fastfetch &>/dev/null; then
        ...
        if confirm_action "Install fastfetch?"; then
            pkg install fastfetch -y 2>/dev/null || echo "Fastfetch not available in repos"
            if command -v fastfetch &>/dev/null; then
                fastfetch
            else
...
show_system_info() {
    ...
    info+="Storage: $(df -h /data 2>/dev/null | awk 'NR==2{print $2" total " $3" used"}' || echo "Unknown")\n"
    info+="RAM: $(free -h 2>/dev/null | awk 'NR==2{print $2" total "$3" used"}' || echo "Unknown")"
...
show_cpu_info() {
    ...
    info+="Cores: $(nproc 2>/dev/null || echo "Unknown")\n"
    info+="CPU Info: $(cat /proc/cpuinfo 2>/dev/null | grep "Hardware" | head -1 | cut -d: -f2 | xargs || echo "Unknown")\n"
    info+="Features: $(cat /proc/cpuinfo 2>/dev/null | grep "Features" | head -1 | cut -d: -f2 | xargs | cut -c1-50 || echo "Unknown")..."
...
show_ram_info() {
    ...
    if command -v free &>/dev/null; then
        local total=$(free -h 2>/dev/null | awk 'NR==2{print $2}')
        local used=$(free -h 2>/dev/null | awk 'NR==2{print $3}')
        local free=$(free -h 2>/dev/null | awk 'NR==2{print $4}')
        info+="Total: $total\n"
        info+="Used: $used\n"
        info+="Free: $free\n"
        info+="Swap: $(free -h 2>/dev/null | awk 'NR==3{print $2" total " $3" used"}' || echo "N/A")"
    else
...
package_menu() {
    while true; do
        ...
        case "$choice" in
            1) show_info "Updating..."; pkg update 2>/dev/null; show_success "Done"; read -p "Enter" ;;
            2) show_info "Upgrading..."; pkg upgrade -y 2>/dev/null; show_success "Done"; read -p "Enter" ;;
            3) show_info "Cleaning..."; pkg clean 2>/dev/null; show_success "Done"; read -p "Enter" ;;
            4) show_info "Autoremoving..."; pkg autoclean 2>/dev/null; show_success "Done"; read -p "Enter" ;;
            5) pkg list-installed 2>/dev/null; read -p "Enter" ;;
            6) read -p "Search: " term; pkg search "$term" 2>/dev/null; read -p "Enter" ;;
            0|7) break ;;
        esac
    done
}
    ...
    while true; do
        ...
        case "$choice" in
            ...
            4)
                ...
                mount 2>/dev/null | grep "/data" || mount 2>/dev/null | head -10
                read -p "Enter"
                ;;
            0|5) break ;;
        esac
    ...
    while true; do
        ...
        case "$choice" in
            1)
                ...
                len="${len:-16}"
                pw=$(tr -dc 'A-Za-z0-9!@#$%^&*' < /dev/urandom 2>/dev/null | head -c "$len")
            ...
            3)
                ...
                echo "MD5: $(echo -n "$text" | md5sum 2>/dev/null | awk '{print $1}')"
                echo "SHA1: $(echo -n "$text" | sha1sum 2>/dev/null | awk '{print $1}')"
                echo "SHA256: $(echo -n "$text" | sha256sum 2>/dev/null | awk '{print $1}')"
                read -p "Enter"
                ;;
            4)
                read -p "Length [16]: " len
                len="${len:-16}"
                rand=$(tr -dc 'A-Za-z0-9' < /dev/urandom 2>/dev/null | head -c "$len")
