...
print_box() {
    ...
    while IFS= read -r line; do
        ...
        if [[ $line_len -le $max_width ]]; then
            processed_content+="$line"$'\n'
        else
            # Wrap long lines
            local wrapped=$(echo "$line" | fold -s -w $max_width 2>/dev/null || echo "$line")
    ...
    if [[ -n "$title" ]]; then
        ...
        local clean_title=$(echo "$title" | sed -E 's/\x1b\[[0-9;]*m//g')
    ...
    while IFS= read -r line; do
        ...
        if [[ $line_len -gt $max_width ]]; then
            clean_line="${clean_line:0:$((max_width-3))}..."
            line_len=$max_width
            printf "${BOLD}${BLUE}│${RESET} %s%*s ${BOLD}${BLUE}│${RESET}\n" "$clean_line" $((max_width - line_len)) ""
        else
            printf "${BOLD}${BLUE}│${RESET} %s%*s ${BOLD}${BLUE}│${RESET}\n" "$line" $((max_width - line_len)) ""
        fi
...
show_temperature() {
    if ! check_command "sensors"; then
        ...
        if ! check_command "sensors"; then
            show_warning "lm-sensors installation failed. Trying fallback..."
            if [[ -f /sys/class/thermal/thermal_zone0/temp ]]; then
                local temp=$(cat /sys/class/thermal/thermal_zone0/temp 2>/dev/null || echo 0)
                print_box "System Temperature" "CPU Temperature: $((temp / 1000))°C"
                return
            fi
        fi
    fi
    ...
    case "$OS" in
        ...
        Termux)
            if command -v termux-wifi-connectioninfo &>/dev/null; then
                ...
                if confirm_action "termux-api not installed. Install?"; then
                    pkg install termux-api -y
                    if command -v termux-wifi-connectioninfo &>/dev/null; then
                        info="$(termux-wifi-connectioninfo 2>/dev/null)"
                    else
                        info="Installation failed. Please install termux-api manually."
                    fi
                else
                    info="termux-api required for Wi-Fi info"
                fi
            fi
            ;;
        *)
    ...
    case "$OS" in
        ...
        Termux)
            if command -v termux-battery-status &>/dev/null; then
                ...
                if confirm_action "Install termux-api for battery info?"; then
                    pkg install termux-api -y
                    if command -v termux-battery-status &>/dev/null; then
                        info+="$(termux-battery-status 2>/dev/null)"
                    else
                        info+="termux-api installation failed"
                    fi
                else
                    info+="termux-api required for battery info"
                fi
            fi
            ;;
        *)
...
show_smart_status() {
    if ! check_command "smartctl"; then
        show_info "smartctl not installed. Installing..."
        auto_install_tool "smartctl" "smartmontools"
        if ! check_command "smartctl"; then
            print_box "SMART Status" "smartctl not available. Please install smartmontools manually."
            return
        fi
    fi
...
# ========================================
# Initialization - WIZARD SKIPPED
# ========================================
initialize() {
    detect_os
    detect_distro
    detect_package_manager
    # first_run_wizard   # <--- WIZARD SKIPPED - Termux detection handled inside
